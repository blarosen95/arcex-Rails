class BaseSerializer
    ...
    def self.attributes_hash(record, fieldset = nil, params = {})
        ...
        raw_attributes.each do |key, value|
            if enums.key?(key)
                raw_attributes[key] = record.read_attribute_before_type_cast(key)
            end
        end

        raw_attributes.transform_keys! { |key| key.to_s.camelcase(:lower).to_sym } if self.transform_method == %i[camelize lower]
    ...
    def self.relationships_hash(record, relationships = nil, fieldset = nil, includes_list = nil, params = {})
        relationships = 
            (relationships || self.relationships_to_serialize || []).reject do
                self.exclude_relationships(record, params).include?(_1.to_sym)
            end
